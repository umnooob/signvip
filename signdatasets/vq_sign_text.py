...
class VQSignTextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while flag:
            ...
            if self.sk_videos_info and self.hamer_videos_info:
                ...
                pose_latents = pose_latents[: self.max_pose_len, :]  # [Frame, pose_dim]
                pose_latents = torch.tensor(pose_latents, dtype=torch.long).flatten()

                pose_latents = torch.cat(
                    [
                        pose_latents,
                        torch.tensor([self.end_idx] * self.pose_size, dtype=torch.long),
                    ],
                    dim=0,
                )  # [pose_len*codes]
                ...
                pose_latents = torch.nn.functional.pad(
                    pose_latents,
                    (0, (self.max_pose_len + 1) * self.pose_size - pose_len),
                    "constant",
                    self.pad_idx,
                )
                sample = dict(
