...
class Upsample2x(nn.Module):
    ...
    def forward(self, x, out_size=None):
        if out_size is not None:
            return self.conv(F.interpolate(x, size=out_size, mode="nearest"))
...
class Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.feature_sizes.append((current_h, current_w))
        
        for _ in range(self.num_resolutions - 1):  # -1 because last level doesn't downsample
    ...
    def forward(self, x):
        ...
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                # print(f"encoder downsample h shape: {h.shape}")
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                # print(f"out size: {self.out_size}")
                # print(f"decoder upsample h shape: {h.shape}")
                h = self.up[i_level].upsample(h, self.out_size[i_level-1])
                # print(f"after decoder upsample h shape: {h.shape}")
