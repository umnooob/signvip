    ...
    def forward(
        ...
        with quantization_context():
            ...
            if self.training and self.commitment_loss_weight > 0.:
                # Remove debug print
                # print(original_input.shape, quantized.shape)
                
                # Ensure shapes match by flattening batch and sequence dimensions
                original_flat = rearrange(original_input, 'b n ... -> (b n) ...')
                quantized_flat = rearrange(quantized, 'b n ... -> (b n) ...')
                
                commit_loss = F.mse_loss(original_flat, quantized_flat.detach(), reduction = 'none')
