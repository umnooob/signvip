...
def process_video(video_path, pose_path, generated_path, output_path, queue):
    try:
        if os.path.exists(output_path):
            video = cv2.VideoCapture(video_path)
            out_video = cv2.VideoCapture(output_path)
            if out_video.isOpened():
                generated_total_frames = int(out_video.get(cv2.CAP_PROP_FRAME_COUNT))
                ...
                if generated_total_frames == total_frames:
                    ...
                    print(
                        f"Removing {output_path} because it has different frame count {generated_total_frames} != {total_frames}"
                    )
        ...
        generated_cap = cv2.VideoCapture(generated_path)

        total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        generated_total_frames = int(generated_cap.get(cv2.CAP_PROP_FRAME_COUNT))

        if total_frames == generated_total_frames:
            return

        # fix the generated video
        ...
        for frame_idx in range(total_frames):
            if frame_idx in video_keypoints.keys():
                ...
                if len(all_verts) > 0:
                    ...
                    generated_cap.set(cv2.CAP_PROP_POS_FRAMES, gen_frame_idx)
                    _, generated_frame = generated_cap.read()
                    out_video.write(generated_frame)
                    gen_frame_idx += 1
                    continue  # Skip the black frame writing
                else:
                    # all black
        ...
        assert (
            gen_frame_idx == generated_total_frames
        ), f"{gen_frame_idx} != {generated_total_frames}"
        out_video.release()
        cap.release()
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Folder with generated videos",
    )
    ...
    with multiprocessing.Pool(processes=multiprocessing.cpu_count()) as pool:
        ...
        args = [
            (
                ...
                os.path.join(args.generated_folder, video_file),
